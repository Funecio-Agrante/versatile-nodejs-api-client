...
const DateInterval = (startTime, endTime) => {
    if (endTime && (endTime - startTime) > 259200)
        endTime = undefined;
...
//await ListGameModes();
const SealedGames = async (startTime, endTime) => {
    let page = 0;
    const gameRecords = [];
    const dateRange = DateInterval(startTime, endTime);
    consolidate: while (true) {
        page++;
        const sealedDataAPI = await apiManager.fetchAPIData('match', { sort: 'end_time', mode_id: 7, end_time: dateRange, page });
        ...
        if (page * sealedDataAPI.perPage >= sealedDataAPI.total)
            break;
    }
    const matchData = [];
...
const PlayerMatches = async (mode_id, playerID, startTime, endTime) => {
    ...
    consolidateWins: while (true) {
        page++;
        const wonMatchesAPI = await apiManager.fetchAPIData('match', { sort: 'end_time', mode_id, player_won: playerID, end_time: dateRange, page });
        if (!wonMatchesAPI.records)
            break; // If there are no records, this will be null
        ...
        if (page * wonMatchesAPI.perPage >= wonMatchesAPI.total)
            break;
    }
    page = 0;
    consolidateLosses: while (true) {
        page++;
        const lostMatchesAPI = await apiManager.fetchAPIData('match', { sort: 'end_time', mode_id, player_lost: playerID, end_time: dateRange, page });
        if (!lostMatchesAPI.records)
            break; // If there are no records, this will be null
        ...
        if (page * lostMatchesAPI.perPage >= lostMatchesAPI.total)
            break;
    }
